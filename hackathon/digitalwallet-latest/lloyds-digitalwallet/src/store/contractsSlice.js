...
export const  fetchContracts = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      // Set default page size to 1000 to fetch all contracts at once
      const fetchParams = { size: 1000, ...params };
      const response = await contractAPI.list(fetchParams);
...
export const deployCustomContract = createAsyncThunk(
  'contracts/deployCustomContract',
  async (formData, { rejectWithValue }) => {
    try {
      const response = await contractAPI.deployCustomContract(formData, {headers: {'Content-Type': 'multipart/form-data'}});
      return response.data;
    } catch (err) {
      return rejectWithValue(err.response?.data || err.message);
    }
...
export const fetchRequestDetails = createAsyncThunk(
  'contracts/fetchRequestDetails',
  async (contractId, { rejectWithValue }) => {
    try {
      const response = await contractAPI.getRequestDetails(contractId);
      return response.data;
    } catch (err) {
      return rejectWithValue(err.response?.data || err.message);
    }
...
export const contractsSlice = createSlice({
  ...
  reducers: {
    ...
    setSwapContractAddress: (state, action) => {
      state.swapContractAddress = action.payload;
    },
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deployCustomContract.pending, (state) => {
      state.isLoading = true;
      state.error = null;
    });
    builder.addCase(deployCustomContract.fulfilled, (state, action) => {
      state.isLoading = false;
      // If the response contains contractType 'swap', store the contract address
      // if (action.payload.contractType === 'swap' && action.payload.contractAddress) {
      //   state.swapContractAddress = action.payload.contractAddress;
      // }
      state.error = null;
    });
    builder.addCase(deployCustomContract.rejected, (state, action) => {
      state.isLoading = false;
      state.error = action.payload || 'Custom contract deployment failed';
    });
...
// Export actions
export const { setContracts, clearError, updatePagination, setSwapContractAddress, setContractAddress } = contractsSlice.actions;
...
export const selectSwapContractAddress = (state) => state.contracts.swapContractAddress;
export const selectContractAddress = (state) => state.contracts.contractAddress;
