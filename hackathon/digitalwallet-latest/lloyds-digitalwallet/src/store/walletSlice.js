...
export const fetchTokens = createAsyncThunk(
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      // Get minted contract info from Redux state
      const state = getState();
      const mintedContractInfo = state.wallet.mintedContractInfo;
      
      const tokenPromises = [];
      
      // Fetch tokens for minted contract if exists
      if (mintedContractInfo && mintedContractInfo.contractAddress) {
        tokenPromises.push(
          walletAPI.getTokens(mintedContractInfo.contractAddress)
            .then(response => ({
              contractAddress: mintedContractInfo.contractAddress,
              contractName: mintedContractInfo.contractName,
...
const initialState = {
  ...
  mintedContractInfo: null, // store single { contractAddress, contractName } mapping
};
...
export const walletSlice = createSlice({
  ...
  reducers: {
    ...
    setMintedContractInfo: (state, action) => {
        // Store single contract info object
        state.mintedContractInfo = action.payload;
    },
    clearMintedContractData: (state) => {
        state.mintedContractAddress = null;
        state.mintedContractInfo = null;
    }
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(buyTokens.fulfilled, (state, action) => {
      ...
      action.payload.requestStatus == "FAILURE" ? state.buyTokensError = 'Failed to buy tokens' : state.buyTokensError = null;
    ...
    builder.addCase(redeemTokens.fulfilled, (state, action) => {
      console.log('Redeem Tokens fulfilled with payload:', action.payload);
      action.payload.requestStatus == "FAILURE" ? state.redeemTokensError = 'Failed to redeem tokens' : state.redeemTokensError = null; // Set initial status
