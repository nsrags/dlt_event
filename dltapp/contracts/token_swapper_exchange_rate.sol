...
contract TokenSwapperWithExchangeRate {
     ...
     */
    function _convertAmount(uint256 amount, uint256 rate) internal pure returns (uint256) {
	...
	function buyMMFTokenWithExchangeRate(address sender, address recipient, uint256 amountA) external {
		...
		uint256 convertedAmountA = _convertAmount(amountA, currentConversionRate);
		
		bool approvedA = IERC20(tokenA).forceApprove(sender, address(this), convertedAmountA);
		require(approvedA, "Swap: TokenA forceApprove failed or not authorized"); 
		
		bool approvedB = IERC20(tokenB).forceApprove(recipient, address(this), amountA);
		require(approvedB, "Swap: TokenB forceApprove failed or not authorized"); 
		// success
		bool successA = IERC20(tokenA).transferFrom(sender, recipient, convertedAmountA);
	...
	function redeemMMFTokenWithExchangeRate(address sender, address recipient, uint256 amountA) external {
		...
		require(approvedA, "Swap: TokenB forceApprove failed or not authorized"); 
		
		bool approvedB = IERC20(tokenA).forceApprove(recipient, address(this), convertedAmountA);
		require(approvedB, "Swap: TokenA forceApprove failed or not authorized"); 
		// success
		bool successA = IERC20(tokenB).transferFrom(sender, recipient, amountA);
		require(successA, "TokenB transferFrom failed");
		
		bool successB = IERC20(tokenA).transferFrom(recipient, sender, convertedAmountA);
