...
import { selectTokens, fetchTokens } from '../../store/walletSlice';
import { DocumentDuplicateIcon } from '@heroicons/react/24/outline';
import LBG_Test from '../../assets/LBG_Test.png';
  ...
  <section className="bg-[#0a4226] text-white rounded shadow-md p-6 text-center w-full max-w-md mx-auto">
      ...
      {tokens && (() => {
        ...
        const items = tokenArray.map((t) => {
          ...
          const raw = t.data?.result || t.data?.balance || t.result || t.balance || (typeof t === 'string' ? t : undefined);
          ...
          return { addr, name, balanceStr, readable };
        });

        return (
          <div className="mt-4 w-[90%] m-auto text-left">
            <label className="block text-sm text-white/90 mb-2">Token Holdings</label>
            <ul className="space-y-2">
              {items.map(it => (
                <li key={it.addr} className="flex items-center justify-between bg-white/5 p-2 rounded border border-white/10">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-sm font-semibold">{Number(it.readable || 0)}</div>
                  </div>
                </li>
              ))}
            </ul>
